...
func TestMarshalUnmarshal(t *testing.T) {
	testcases := map[string]struct {
		...
		"multiple ports": {
			mutate: func(es mcs.ExportSpec) *mcs.ExportSpec {
				new_port_81 := mcsv1a1.ServicePort{Port: 81, Name: "http", Protocol: corev1.ProtocolTCP}
				new_port_82 := mcsv1a1.ServicePort{Port: 82, Name: "http", Protocol: corev1.ProtocolTCP}
	...
	for name, test := range testcases {
		...
		assertions.NoError(err)
		convertTimesToUTC(&test.expected.CreatedAt, &actual.CreatedAt)
		assertions.Equal(test.expected, actual)
	}
}

func convertTimesToUTC(t1 *metav1.Time, t2 *metav1.Time) {
	loc, _ := time.LoadLocation("UTC")
	t1.Time = t1.Time.In(loc).Truncate(time.Second)
	t2.Time = t2.Time.In(loc).Truncate(time.Millisecond)
}
